...
int main() {
    ...
    for (int iter = 0; iter < numIterations; iter++) {
        ...
        // Paralléliser cette boucle pour calculer les clusters en parallèle
        #pragma omp parallel // Début de la région parallèle
        {
            int threadID = omp_get_thread_num(); // Obtenir l'ID du thread actuel
            int numThreads = omp_get_num_threads(); // Obtenir le nombre total de threads

            printf("Thread %d / %d : Starting iteration %d\n", threadID, numThreads, iter);

            #pragma omp for // Parallélisation de la boucle suivante
            for (int i = 0; i < numDataPoints; i++) {
                ...
                #pragma omp atomic // Assurer l'atomicité de cette opération
                clusterSizes[closestCentroid]++;
            }

            printf("Thread %d / %d : Completed iteration %d\n", threadID, numThreads, iter);
